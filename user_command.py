...
def add(bot, update, tpye,date=[]):
    try:
        for n in user_date.for_group:
            if tpye==n.name and n.admin.user_list.inornot(update.message.chat_id):
                n.add(date)
...
def clean(bot,update,args):
    try:
        for n in user_date.for_group:
            if args[0]==n.name and n.admin.user_list.inornot(update.message.chat_id) and args[1]==config.keyuser:
               n.clean()
               return
            update.message.reply_text('喵？并没有名叫' + type + '的模块块哦？0w0')
    except TypeError:
