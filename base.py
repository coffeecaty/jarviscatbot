...
def message(bot, update):
    ...
    if update.message.chat_id == config.yourid:
        ...
        bot.sendMessage(update.message.chat_id, text='小猫虽然听不懂你在说什么，但是小猫爱大熊！小猫会问问大猫怎么回答你的！',
                        reply_to_message_id=update.message.message_id)
    else:
        bot.sendMessage(update.message.chat_id, text='小猫虽然听不懂你在说什么，但是小猫爱大熊！',
                        reply_to_message_id=update.message.message_id)
        bot.sendMessage(update.message.chat_id,
                        text='如果想给coffeecaty或者summyxy留言，请使用/stc（say to coffeecaty）或者/sts（say to summyxy）')
...
def stc(bot, update, args):
    ...
    if len(args) > 0:
        ...
        bot.sendMessage(config.yourid, text='from:@' + update.message.from_user.username + ':' + message + '(at ' + str(
...
def sts(bot, update, args):
    ...
    if len(args) > 0:
        ...
        bot.sendMessage(config.loverid,
                ...
                if update.message.chat_id not in n.list:
                    ...
                    for m in n.send_to, list:
                        if m not in n.send_to.mu + config.alluser.mu:
                            bot.sendMessage(m,
                                            text='@' + update.message.from_user.username + '向您申请' + n.name + '模块的使用权限(at ' + str(
                                                update.message.date + timedelta(hours=8)) + 'UTC+8:00)')
